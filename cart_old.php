  ...
  <div class="row justify-content-center">
    ...
    <div class="col-lg-8 text-center d-none" id="emptyCartMessage">
      <div class="text-muted my-5">
        ...
        <p class="text-secondary">Add some products to start checking out</p>
      ...
      <div class="border rounded">
        ...
        <div class="p-3">
          ...
          <div class="d-grid gap-2 mt-4">
            <button id="paypalCheckoutBtn" class="btn btn-primary text-white">
              <i class="fab fa-paypal fa-lg me-2"></i> Pay with PayPal
            </button>
            <button id="stripeCheckoutBtn" class="btn btn-dark text-white">
              <i class="fab fa-stripe fa-lg me-2"></i> Pay with Stripe
            </button>
          </div>
        </div>
      </div>
...
<div id="emptyCartMessage" class="text-center text-muted d-none">
  <img src="assets/images/empty-cart.png" alt="Empty Cart" style="max-width:120px;" class="mb-3">
  <h5>Your cart is empty</h5>
  <p>Looks like you haven’t added anything yet.</p>
</div>

<script>
  let cartProductIds = [];         // All product IDs in cart
  let cartProductQtys = [];        // Matching quantities for each product_id
  ...
  function renderCartItem(item) {
    ...
    <tr class="cart-item" data-id="${item.cart_item_id}">
      ...
      <td>
        <img src="admin/ajax/${item.image_url || 'https://via.placeholder.com/170'}" alt="${item.title}" class="img-fluid rounded" style="max-width:170px;"/>
      </td>
  ...
  /*function loadCart() {
    $.getJSON('ajax/cart_get.php', function (items) {
      ...
      if (!items || items.length === 0) {
        // Hide cart table and totals, show empty message
      ...
      // Debug
      console.log("Cart loaded:", cartProductIds, cartProductQtys);
    });
  }*/
  ...
  function loadCart() {
    $.getJSON('ajax/cart_get.php', function (items) {
      ...
      if (!hasItems) {
        console.log('if no items in cart');
        // No items in cart
        $('#cartSection').addClass('d-none');
        $('#cartTotalSection').addClass('d-none');
        $('#emptyCartMessage').removeClass('d-none');
      ...
      // Always update totals (even if 0)
      $('#subtotal').text('$' + subtotal.toFixed(2));
      $('#grandtotal').text('$' + subtotal.toFixed(2));
      ...
      $('#cartTotalSection').addClass('d-none');
      $('#emptyCartMessage').removeClass('d-none')
        .html('<p class="text-danger">Failed to load cart. Please try again later.</p>');
    });
  }
  ...
  function startCheckout(buttonId, gateway = 'paypal') {
    ...
    $btn.attr('disabled', true).html('<span class="spinner-border spinner-border-sm"></span> Redirecting...');

    // Set backend URL depending on gateway
    let ajaxUrl = gateway === 'stripe'
      ? 'ajax/buy_now_stripe.php'
      : 'ajax/buy_now_paypal.php';
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data && (data.checkoutUrl || data.approveUrl)) {
          // Redirect depending on key
          window.location.href = data.checkoutUrl || data.approveUrl;
        } else {
          const message = data.error || data.message || "Unknown error";
          alert(`${gateway.toUpperCase()} error: ${message}`);
          console.error(`${gateway} response:`, data);
          $btn.prop('disabled', false).html(originalText);
        }
      },
      error: function (xhr, textStatus, errorThrown) {
        let msg = `❌ ${gateway.toUpperCase()} AJAX error:\nStatus: ${textStatus}\nError: ${errorThrown}`;
        if (xhr.responseText) {
          try {
            const json = JSON.parse(xhr.responseText);
            msg += "\nMessage: " + (json.message || json.error || xhr.responseText);
          } catch (e) {
            msg += "\nRaw:\n" + xhr.responseText;
          }
        }
  ...
  // Bind buttons
  $('#paypalCheckoutBtn').click(function () {
    startCheckout('paypalCheckoutBtn', 'paypal');
  });

  $('#stripeCheckoutBtn').click(function () {
    startCheckout('stripeCheckoutBtn', 'stripe');
  });
...
<div class="col">
  <div class="product-card bg-white">
    ...
    <img src="admin/ajax/${p.image || '../../images/octopus.webp'}" class="product-img" alt="${p.title}">

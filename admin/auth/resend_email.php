...
$email = "amazingpuzzle70@gmail.com";
if (!$email) {
    echo json_encode(['status' => 'error', 'message' => 'Invalid email address.']);
    exit;
}
...
$resetLink = BASE_URL . 'auth/verify_token.php?token=' . urlencode($token);



// Send email
//$mailer = new Mailer();
//$response = $mailer->sendLinkEmail($email, 'verification', $resetLink);
...
/*if ($response === 1) {
    ...
    echo json_encode(['status' => 0, 'message' => 'Failed to send email', 'debug' => $response]);
}*/
echo json_encode(['status' => 1, 'message' => 'Email sent successfully']);
